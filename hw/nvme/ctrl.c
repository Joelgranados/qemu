...
static uint16_t nvme_map_addr_dma(NvmeCtrl *n, QEMUIOVector *iov, DMADirection dir,
                                  hwaddr addr, size_t len)
{
    ...
    while (len > 0) {
        dma_len = len;
        mem = dma_memory_map(as, addr, &dma_len, dir, MEMTXATTRS_UNSPECIFIED);

        if (!mem)
            return NVME_INVALID_SGL_SEG_DESCR;

        qemu_iovec_add(iov, mem, dma_len);
        addr += dma_len;
        len -= dma_len;
    }
    return NVME_SUCCESS;
...
static uint16_t nvme_map_addr(NvmeCtrl *n, NvmeSg *sg, hwaddr addr, size_t len)
{
    ...
    if (nvme_addr_is_cmb(n, addr)) {
        ...
        return nvme_map_addr_dma(n, &sg->iov, sg->dir, addr, len);
    }
